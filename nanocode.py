...
REQUEST_TIMEOUT = parse_int_env("NANOCODE_HTTP_TIMEOUT", 30, minimum=15)
PROVIDERS = {
    "anthropic": {
        "label": "Anthropic",
...
def detect_provider():
    explicit = os.environ.get("NANOCODE_PROVIDER", "").strip().lower().replace("z_ai", "zai")
...
def write_text(path, content):
    with open(path, "w", encoding="utf-8") as file:
        file.write(content)
def read(args):
    lines = read_text(args["path"]).splitlines(keepends=True)
    offset = args.get("offset", 0)
    limit = args.get("limit", len(lines))
    return "".join(f"{offset + i + 1:4}| {line}" for i, line in enumerate(lines[offset : offset + limit]))
def write(args):
...
def edit(args):
    ...
    write_text(args["path"], text.replace(old, new) if args.get("all") else text.replace(old, new, 1))
    return "ok"
def glob(args):
...
def bash(args):
    try:
        ...
        output = err.stdout.decode("utf-8", errors="replace") if isinstance(err.stdout, bytes) else (err.stdout or "")
...
def normalize_openai_content(content):
    ...
    if not isinstance(content, list):
        return []
    text = "".join(item if isinstance(item, str) else item.get("text", "") for item in content if isinstance(item, (str, dict)) and (not isinstance(item, dict) or item.get("type") == "text"))
...
def extract_usage_tuple(response):
    ...
    return tuple(value if isinstance(value, int) else None for value in (input_tokens, output_tokens, total_tokens))
...
def parse_response(provider, response):
    if PROVIDERS[provider]["style"] == "anthropic":
        texts, tool_calls, content = [], [], response.get("content", [])
        for block in content:
            ...
            if block.get("type") == "text" and isinstance(block.get("text"), str):
                ...
                tool_calls.append({"name": block.get("name", ""), "args": block.get("input", {}), "id": block.get("id", "")})
        return {
    ...
    for tool_call in raw_tool_calls:
        ...
        tool_calls.append({"name": function_call.get("name", ""), "args": parse_tool_args(function_call.get("arguments")), "id": tool_call.get("id", "")})
    message_texts = normalize_openai_content(message.get("content"))
    plan_text = extract_zai_plan(choice, message) if provider == "zai" else ""
    payload = {"role": "assistant", "content": "".join(message_texts), **({"tool_calls": raw_tool_calls} if raw_tool_calls else {})}
    return {
        "assistant_text": [plan_text, *message_texts] if plan_text else message_texts,
        "tool_calls": tool_calls,
        "usage": extract_usage_tuple(response),
        "assistant_payload": payload,
    }
def make_schema(provider):
    openai_style, schemas = PROVIDERS[provider]["style"] == "openai", []
...
def build_request(messages, system_prompt, provider):
    payload = {"model": MODEL, "max_tokens": 8192, "messages": messages, "tools": make_schema(provider)}
...
def provider_headers(provider):
    ...
    headers[cfg["auth_header"]] = f"{cfg['auth_prefix']}{os.environ.get(cfg['key_env'], '')}"
    return headers
def provider_api_url(provider):
    url = PROVIDERS[provider]["api_url"].rstrip("/")
    return f"{url}/chat/completions" if PROVIDERS[provider]["style"] == "openai" and not url.endswith("/chat/completions") else url
def decode_json_bytes(data):
    ...
    try:
        ...
        raise RuntimeError(f"Non-JSON response from {PROVIDER_LABEL}: {snippet or '<empty>'}") from err
def request_json(request):
    try:
        ...
        raise RuntimeError(f"HTTP {err.code} from {PROVIDER_LABEL}: {detail}") from err
    except urllib.error.URLError as err:
        raise RuntimeError(f"Network error from {PROVIDER_LABEL}: {err.reason}") from err
def dry_run_response(provider):
    cfg, api_url = PROVIDERS[provider], provider_api_url(provider)
    ...
    if cfg["style"] == "openai":
        ...
        return {"choices": [{"message": message}], "usage": {"prompt_tokens": 12, "completion_tokens": 8, "total_tokens": 20}}
    return {"content": [{"type": "text", "text": text}], "usage": {"input_tokens": 12, "output_tokens": 8, "total_tokens": 20}}
...
def usage_dict(usage_tuple):
    if not isinstance(usage_tuple, tuple) or len(usage_tuple) != 3:
        return None
    values = dict(zip(("input", "output", "total"), usage_tuple))
...
def add_usage(bucket, usage):
    ...
    for key in ("input", "output", "total"):
        if usage[key] is not None:
            bucket[key] = usage[key] if bucket[key] is None else bucket[key] + usage[key]
def usage_parts(bucket):
    return [f"{label} {bucket[key]}" for key, label in (("input", "in"), ("output", "out"), ("total", "total")) if bucket[key] is not None]
def separator():
    try:
...
def tool_result_message(provider, tool_call, result):
    if PROVIDERS[provider]["style"] == "anthropic":
        return {"role": "user", "content": [{"type": "tool_result", "tool_use_id": tool_call["id"], "content": result}]}
...
def initial_messages(system_prompt):
    return [{"role": "system", "content": system_prompt}] if PROVIDER_CFG["system_message"] else []
def run_turn(messages, system_prompt, session_usage):
    ...
    while True:
        ...
        for tool_call in parsed["tool_calls"]:
            result = execute_tool(tool_call.get("name", ""), tool_call.get("args") or {})
...
def main():
    try:
        ...
        return
    print(f"{BOLD}nanocode{RESET} | {DIM}{MODEL} ({PROVIDER_LABEL}) | {os.getcwd()}{RESET}\n")
    print_help()
    system_prompt = f"Concise coding assistant. cwd: {os.getcwd()}"
    messages, session_usage = initial_messages(system_prompt), new_usage_bucket()
    while True:
        try:
            print(separator())
            ...
            print_usage_summary(run_turn(messages, system_prompt, session_usage), session_usage)
            print()
